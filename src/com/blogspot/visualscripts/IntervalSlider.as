package com.blogspot.visualscripts {
	...
	*/
	import com.blogspot.visualscripts.skins.IntervalSliderSkin;
	...
	/**
	 *  Dispatched when thumb drag is complete (either thumb)
	 *
	 *  @eventType flash.events.SliderEvent.THUMB_RELEASE
	 */
	...
	public class IntervalSlider extends SkinnableComponent{
		...
		public function IntervalSlider(){
			super();
			this.setStyle("skinClass",com.blogspot.visualscripts.skins.IntervalSliderSkin);
		}
		...
		private function dragThumb1( e:MouseEvent ):void{
			...
			var newx:Number = thumb1x+stage.mouseX-oldMouseX;
			
			if(thumb2x - thumb1x < 100){
				trace('TOO CLOSE!');
				var distanceTraveled:Number = Math.abs(thumb1x - newx);
				distanceTraveled = distanceTraveled/3;
				if(newx > thumb1x){
					newx = thumb1x + distanceTraveled;
				}else{
					newx = thumb1x - distanceTraveled;
				}
			}
			...
			var newT1val:Number = nearestValidValue(thumb1x*(max-min)/(width)+min,snapInterval,min,pendingT2Val );
		...
		private function dragThumb2( e:MouseEvent ):void{
			var newx:Number = thumb2x+stage.mouseX-oldMouseX;
			if(thumb2x - thumb1x < 100){
				trace('TOO CLOSE!');
				var distanceTraveled:Number = Math.abs(thumb2x - newx);
				distanceTraveled = distanceTraveled/3;
				if(newx > thumb2x){
					newx = thumb2x + distanceTraveled;
				}else{
					newx = thumb2x - distanceTraveled;
				}
			}
			...
			var newT2val:Number = nearestValidValue( (thumb2x-thumb2.width)*(max-min)/(width)+min,snapInterval,pendingT1Val,max);
		...
		override protected function updateDisplayList(unscaledWidth:Number,unscaledHeight:Number):void{
			...
			var valueinPixels:Number = (width)/(max-min);
			//thumb1.x = (pendingT1Val-min)*valueinPixels - thumb1.width/2;
			//thumb2.x = (pendingT2Val-min)*valueinPixels+thumb1.width - thumb1.width/2;
			thumb1.x = (pendingT1Val-min)*valueinPixels;
			thumb2.x = (pendingT2Val-min)*valueinPixels;
			//trace ( "UPDATE" )
		}
