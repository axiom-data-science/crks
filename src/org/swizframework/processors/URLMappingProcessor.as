package org.swizframework.processors
{
	...
	public class URLMappingProcessor extends BaseMetadataProcessor
	{
		...
		override public function setUpMetadataTag( metadataTag:IMetadataTag, bean:Bean ):void
		{
			...
			//Alert.show('SETTING UP METADATA! ' + metadataTag.asTag);
		...
		{
			//Alert.show(eventType + ' ' + mediateEventHandler);
		...
		protected function mediateEventHandler( event:Event ):void
		{
			...
			var mediate:IMetadataTag;
			
			//FIX FOR EVENTHANDLER METADATA
			if ( urlMapping.host.hasMetadataTagByName( "Mediate" )){
				mediate = urlMapping.host.getMetadataTagByName( "Mediate");
			}
			if(urlMapping.host.hasMetadataTagByName( "EventHandler" ) ){
				mediate = urlMapping.host.getMetadataTagByName( "EventHandler");
			}
			
			var args:Array = [];
			if(mediate){
				args = mediate.hasArg( "properties" ) ? getEventArgs( event, mediate.getArg( "properties" ).value.split( /\s*,\s*/ ) ) : null;
			}
			...
			if( urlMapping != null )
			{
				...
				if( args != null )
				{
					for ( var i:int = 0; i < args.length; i++ )
					{
						...
						url = url.replace( new RegExp( "\\{" + i + "\\}", "g" ), _customEscape(args[ i ]) );
					}
				}
		...
		//ALLOWS ME TO HAVE LONG URLS WITH PARAM PAIRINGS
		private function _customEscape($str:String):String{
			if($str){
				$str = $str.replace(/\s+/g,'+');
			}
			return $str;
